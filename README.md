...
- **Authors tab** — derived from the search results (unique author names → a query for that author's books), since the catalog has no author navigation feed.
...
`OpdsService.catalogBase` is the only catalog-specific line; any OPDS 1 catalog with an OpenSearch `query=` endpoint works.
